...
func buildSearchCache(ctx context.Context, db *database.DB, cfg *config.Config, log zerolog.Logger) *registry.SearchCache {
	...
	if fps, err := defsFingerprints(dropinDir(cfg)); err != nil {
		log.Warn().Err(err).Msg("computing definition fingerprints failed; skipping def-change cache expiry")
	} else if err := sc.EnsureDefsFingerprints(ctx, fps); err != nil {
		log.Warn().Err(err).Msg("checking definition fingerprints failed; cache may serve stale-shape results after a def update")
	}
	return sc
}
