...
func newSabnzbd(c domain.DownloadClient, secret string, client *http.Client) (Driver, error) {
	settings := deref(c.Settings.Sabnzbd)
...
func (d *sabnzbdDriver) Add(ctx context.Context, p Payload) error {
	...
	if len(p.Bytes) > 0 {
		return d.addFile(ctx, p)
	}
	if p.URL == "" {
		return fmt.Errorf("download: sabnzbd: %w", ErrURLRequired)
	}

	resp, err := d.client.AddFromUrl(ctx, sabnzbd.AddNzbRequest{Url: p.URL, Category: d.defaultCategory})
	if err != nil {
		...
		// request URL — ScrubURLError drops it entirely before addURLError scrubs
		// any literal URL out of the text.
		return addURLError("download: sabnzbd: add nzb from", p.URL, apphttp.ScrubURLError(err))
	}
	if resp.ErrorMsg != "" {
...
// scrubbing.
func (d *sabnzbdDriver) addFile(ctx context.Context, p Payload) error {
