...
// base-URL/type/category map — once.
type Deps struct {
...
func Execute(ctx context.Context, def *loader.Definition, query Query, session *login.Session, doer Doer, deps Deps) ([]*normalizer.Release, error) {
	...
	for i := range reqs {
		...
		// redirect-resolved response's WIRE Content-Type, not the def's declared
		// respType — Jackett reads WebResult.Headers["Content-Type"].
		if looksLoggedOut(def, decoded, sr.header.Get("Content-Type"), query, deps) {
