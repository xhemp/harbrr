...
// fakeInner is a torznab.Indexer test double. It counts Search calls, can block on
...
func testCache(t *testing.T, ttl ttlConfig, refreshPct int) (*SearchCache, int64, *atomic.Pointer[time.Time]) {
	...
	clk.Store(&now)
	sc := NewSearchCache(db, ttl, refreshPct, func() time.Time { return *clk.Load() }, zerolog.Nop())
...
func TestBypassForcesInnerAndWritesBack(t *testing.T) {
	...
	bypassCtx := torznab.WithCacheBypass(context.Background())
...
func waitForTitle(t *testing.T, idx torznab.Indexer, q search.Query, want string) {
	t.Helper()
