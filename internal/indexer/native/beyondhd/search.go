...
func (d *driver) buildRequest(q search.Query) ([]byte, error) {
	...
	if err != nil {
		...
		// before it can surface — via ScrubErr, so the error chain stays intact for
		// errors.Is/As while the displayed message is redacted.
		return nil, fmt.Errorf("beyondhd: build request body: %w", d.ScrubErr(err))
	}
	return body, nil
}
