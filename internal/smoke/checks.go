...
func harbrrParity(ctx context.Context, c *http.Client, cfg Config, slug, primary, fallback string) ([]Result, string, string) {
	res, status, err := HarbrrSearch(ctx, c, cfg.HarbrrURL, cfg.HarbrrKey, slug, primary, true)
	if err != nil {
	...
	query := primary
	if len(res) == 0 {
		if res2, s2, err2 := HarbrrSearch(ctx, c, cfg.HarbrrURL, cfg.HarbrrKey, slug, fallback, true); err2 == nil && s2 == http.StatusOK {
